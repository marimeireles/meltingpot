...
for agt in agents:
    ...
    params = ConvPolicy(act_dim).init(key,
        jnp.zeros((1, *obs_shape), jnp.float32)
    )
...
def collect_batch_per_agent(steps=STEPS_PER_IT):
    ...
    while len(buffers[agent0]["obs"]) < steps:
        ...
        for agt, raw in obs_dict.items():
            ...
            # sample action
            logits, v = ConvPolicy(act_dim).apply(
                nets[agt], x, mutable=False
            )
        ...
        for agt in agents:
            buffers[agt]["rew"].append(
                jnp.array(r_dict.get(agt, 0.0), jnp.float32)
            )
